/*
  ...
  ------------------------------------------------------------------------------
   ...
   Modified by: KBE / 2014.07.03 to be used with the BlackFin DSP implementation
...
static float coefficients[6]; // = {b0, b1, b2, a0, a1, a2}
static int coef_fixed[6]; // = {b0, b1, b2, a0, a1, a2}
								 ...
								 float a2)
{
    ...
    // Scaling of coefficients if greater than 1
    /*
    for (i = 0; i < 6; i++)
    {
        absCoeff = 	abs(coefficients[i]);
    	if (max < absCoeff)
    	   max = absCoeff;
    }
    ...
    if (max > 1.0)
    {
        for (i = 0; i < 6; i++)
        {
        	coefficients[i] = coefficients[i]/max;
        }
    }
    */
